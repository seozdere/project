...
function replaceLegacyParentMessages(root = document) {
    if (!root || typeof root.querySelectorAll !== 'function') return;

    const replacements = new Map([
        ['Öğrenci listesi henüz oluşturulmamış.', 'Henüz ödev girişi yok.'],
        ['Ogrenci listesi henuz olusturulmamis.', 'Henüz ödev girişi yok.'],
        ['HenÃ¼z Ã¶dev giriÅŸi yok.', 'Henüz ödev girişi yok.'],
        ['Ã–ÄŸrenciler', 'Öğrenciler']
    ]);
...
function renderParentNoticeBar() {
    ...
    const expiryText = notice.expires_at
        ? ` <span style="opacity:.8;font-weight:500;">Bitiş: ${new Date(notice.expires_at).toLocaleString('tr-TR', {
            day: '2-digit',
            month: '2-digit',
            hour: '2-digit',
            minute: '2-digit'
    ...
    bar.innerHTML = `<span style="margin-right:8px;">📣</span>${_htmlEncode(notice.text)}${expiryText}`;
...
function renderExcel() {
    ...
    if (!table) return;
    const dates = Object.keys(records).sort().filter(date =>
        students.some(student => records[date]?.[student] !== undefined)
    );
    if (!students.length) {
        table.querySelector('thead').innerHTML = '<tr><th class="sticky-col sticky-header student-header">Öğrenciler</th></tr>';
        table.querySelector('tbody').innerHTML = '<tr><td colspan="99" style="text-align:center;padding:60px;color:var(--p-muted);">Henüz ödev girişi yok.</td></tr>';
        return;
    }

    if (!dates.length) {
        table.querySelector('thead').innerHTML = '<tr><th class="sticky-col sticky-header student-header">Öğrenciler</th><th class="sticky-header date-header">Durum</th></tr>';
        let emptyBody = '';
        students.forEach(student => {
            ...
            emptyBody += `<tr>
                ...
                <td class="cell-empty" style="color:var(--p-muted);font-weight:600;">Henüz ödev girişi yok</td>
            </tr>`;
        });
        table.querySelector('tbody').innerHTML = emptyBody;
        return;
    }
